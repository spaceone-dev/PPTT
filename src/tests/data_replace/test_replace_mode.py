...
class FontReplaceTestCase(PPTTTestCase):
    master_slide = os.path.join(BASE_DIR, 'text_replace.pptx')

    # def test_not_change_font(self):
    #     ms = self.get_master_ppt()
    #     slide_pos = 1
    #     real_slide_pos = slide_pos - 1
    #     origin_slide = ms.slides[real_slide_pos]
    ...
    #
    #     body_shape = self.find_shape(slide, body_shape_name)
    #     body_font = body_shape.text_frame.paragraphs[0].font
    #     body_color = body_font.color.rgb
    #
    #     body_hex_color = contents[title_shape_name]['text']['font']['color'][1:]
    #     self.assertEqual(RGBColor.from_string(body_hex_color), body_color)

    def test_replace_text_with_font(self):
        ms = self.get_master_ppt()
        slide_pos = 1
        real_slide_pos = slide_pos - 1
        origin_slide = ms.slides[real_slide_pos]
